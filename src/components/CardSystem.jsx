  ...
  fetchDrinksRandom = () => {
    ...
    axios
      ...
      .then((quote) => {
        this.setState({ drinkIngredients: [] });
        this.setState({
          resultDrinks: [quote.drinks[0]],
          drinkIngredients: this.getAllIngre(quote.drinks[0]),
        });
      });
  };
  ...
  render() {
    return (
      <div>
        ...
        <section id="cardCont">
          {this.state.resultDrinks.map((drink, index) => (
            <Card
              drink={drink}
              key={index}
              drinkIngredients={this.getAllIngre(drink)}
            />
          ))}
        </section>
      </div>
